...
int main()
{
    ...
    const int L1_input_channels = nr_frames_strobed * nr_color_channels; // color channels
                ...
                if (frame_g >= nr_frames_strobed - 1) // Wait until all 4 images is up there in the game after start
                {
                    ...
                    for (int i = 0; i < L1_input_channels; i++)
                    {
                        for (int j = 0; j < L1_tensor_in_size; j++)
                        {
                            ...
                            float pixelValue = replay_grapics_buffert.at<float>(pixel_height * (frame_g - (nr_frames_strobed - 1) + i) + row, col + pixel_width * g_replay_nr);
